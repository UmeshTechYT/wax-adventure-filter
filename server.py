...
def get_asset_attributes(asset):
    """Flatten common AtomicAssets fields used by adventure modifiers."""
    attrs = {}
    for key in ["name", "cardname", "shine", "schema", "movcost", "movement_cost", "collection", "template"]:
        value = asset_field(asset, key)
        if value not in (None, ""):
            attrs[key.lower()] = str(value)

    # AtomicAssets can expose attributes under data/immutable/mutable/template data.
    sources = [
        asset.get("data") or {},
        asset.get("immutable_data") or {},
        asset.get("mutable_data") or {},
        (asset.get("template") or {}).get("immutable_data") or {},
        (asset.get("template") or {}).get("data") or {},
    ]
    for source in sources:
        for k, v in source.items():
            if v not in (None, ""):
                attrs.setdefault(str(k).lower(), str(v))
    return attrs


def score_suggestion_for_asset(asset, parameters):
    """Calculate explicit cardname/shine/schema matches from mods.parameters.
    movcost is only matched when the NFT exposes a numeric movcost value.
    """
    ...
    for mod in parameters:
        if not isinstance(mod, dict) or str(mod.get("mod_type", "score")).lower() != "score":
            continue
        affix = str(mod.get("affix_type", "")).lower()
        expected = mod.get("affix_string", "")
        ...
        if affix == "cardname":
            ...
            actual = attrs.get("movcost", attrs.get("movement_cost", ""))
            try:
                matched = actual != "" and float(actual) == float(value)
            except (TypeError, ValueError):
                matched = False
        ...
        if matched:
            ...
            matches.append({
                "type": affix,
                "value": str(expected) if expected not in (None, "") else str(value),
                "bonus": bonus,
            })
