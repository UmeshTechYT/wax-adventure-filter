...
def _parse_adventure_time(value):
    ...
    if isinstance(value, dict):
        # Handle EOS/chain time objects such as {"sec": ..., "nsec": ...}
        for key in ("sec", "seconds", "timestamp", "time", "value"):
            if key in value:
                parsed = _parse_adventure_time(value.get(key))
                if parsed:
                    return parsed
        return None

    text = str(value).strip()
    # Numeric timestamps may arrive as strings from JSON/table APIs.
    try:
        if re.fullmatch(r"\d+(?:\.\d+)?", text):
            return _parse_adventure_time(float(text))
    except (TypeError, ValueError):
        pass
    if text.endswith("Z"):
        text = text[:-1] + "+00:00"
...
def _find_adventure_time(row, kind="start"):
    ...
    start_keys = {
        ...
        "scheduled_start", "enter_start", "enterstart", "from", "date_start", "startdate", "start_date"
    }
    end_keys = {
        "end", "endtime", "end_time", "finishtime", "finish_time",
        "closingtime", "closing_time", "endsat", "ends_at", "scheduledend",
        "scheduled_end", "enter_end", "enterend", "deadline", "deadlinetime", "deadline_time",
        "submissiondeadline", "submission_deadline", "expiresat", "expires_at",
        "expiry", "expirytime", "expiry_time", "to", "date_end", "enddate", "end_date"
    }
    if kind == "enter_start":
        wanted = {"enterstart"}
    elif kind == "enter_end":
...
def _adventure_summary(row):
    ...
    return {
        ...
        "enter_start": (_find_adventure_time(row, "enter_start").isoformat() if _find_adventure_time(row, "enter_start") else None),
        "enter_end": (_find_adventure_time(row, "enter_end").isoformat() if _find_adventure_time(row, "enter_end") else None),
    }
...
def adventure():
    ...
    try:
        ...
        current_info = _adventure_summary(current_row) if current_row else None

        # Adventures repeat every 23 hours. Use the current adventure's
        # enter_start as the source and calculate the next start from it.
        next_row = None
        next_info = None
        if current_row:
            current_start = _find_adventure_time(current_row, "enter_start")
            if current_start:
                next_start = current_start + __import__("datetime").timedelta(hours=23)
                higher = [r for r in all_rows if _adventure_id(r) > int(aid)]
                if higher:
                    next_row = min(higher, key=_adventure_id)
                    next_info = _adventure_summary(next_row)
                else:
                    next_info = {
                        "adventure_id": int(aid) + 1,
                        "title": "Next Adventure",
                        "start_time": next_start.isoformat(),
                        "end_time": None,
                        "enter_start": next_start.isoformat(),
                        "enter_end": None,
                    }
                # Always use the calculated 23-hour start for the countdown.
                if next_info:
                    next_info["start_time"] = next_start.isoformat()
                    next_info["enter_start"] = next_start.isoformat()
        return jsonify({
            "adventure_id": aid,
            "count": len(rows),
            "rows": rows,
            "current_adventure": current_info,
...
def top_users_nfts():
    ...
    try:
        ...
        # Mission Control Adventure scores are multiplicative, not additive.
        # Official Adventure documentation describes:
        #   Final Score = 100 * Π(1 + bonus_percent / 100)
        # Only objectives that are actually unlocked for the player contribute.
        # We infer the currently active objective set from the stored highest score
        # and the current winner's NFT coverage. This prevents locked objectives
        # from being treated as active just because their modifier exists in mods.
        def score_from_mask(mask):
            value = 100.0
            for idx, bonus in enumerate(modifier_values):
                if mask & (1 << idx):
                    value *= (1.0 + bonus / 100.0)
            return value

        # Find the objective subset whose multiplicative score is closest to the
        # stored highest score. Prefer subsets that are fully covered by the
        # current winner NFTs when scores are equally close.
        covered_mask = current_mask
        target_score = float(highest)
        inferred_mask = 0
        inferred_error = float("inf")
        inferred_covered = -1
        nmods = len(modifier_values)
        if nmods <= 20:
            for mask in range(1 << nmods):
                projected = score_from_mask(mask)
                error = abs(projected - target_score)
                covered = (mask & covered_mask).bit_count()
                if (
                    error < inferred_error - 1e-9
                    or (abs(error - inferred_error) <= 1e-9 and covered > inferred_covered)
                ):
                    inferred_error = error
                    inferred_mask = mask
                    inferred_covered = covered

        # If the inferred score is materially inconsistent with the stored score,
        # fall back to the modifiers actually covered by the current winner NFTs.
        # This is safer than inventing bonuses that cannot be triggered by the
        # current lineup.
        if abs(score_from_mask(inferred_mask) - target_score) > max(2.0, target_score * 0.02):
            inferred_mask = covered_mask

        active_modifier_mask = inferred_mask
        current_modifier_score = score_from_mask(current_mask & active_modifier_mask)

        # Evaluate every 3-NFT lineup using the real multiplicative formula.
        # A modifier is counted once if ANY of the three NFTs activates it.
        best_states = {(0, 0): {"score": 100.0, "items": []}}
        for item_index, item in enumerate(candidate_list):
            item_mask = int(item.get("_mask", 0)) & active_modifier_mask
            snapshot = list(best_states.items())
            for (count, mask), state in snapshot:
                if count >= 3:
                    continue
                new_mask = mask | item_mask
                new_score = score_from_mask(new_mask)
        ...
        if best_combo:
            projected_score = best_combo["score"]
            replacement_delta = projected_score - float(highest)
        ...
        # Never present an equal/worse lineup as a recommendation.
        if projected_score <= float(highest) + 0.5:
            best_three = []
            projected_score = float(highest)
            replacement_delta = 0.0
        ...
        for mod in parameters:
            ...
            idx = len(available_mods)
            available_mods.append({
                "type": affix,
                "value": str(label),
                "bonus": bonus,
                "covered": bool(matched_nfts),
                "active": bool(active_modifier_mask & (1 << idx)),
        ...
        return jsonify({
            ...
            "possible": best_three,
            "best_score": round(float(projected_score), 2),
            "active_modifier_indexes": [i for i in range(len(score_parameters)) if active_modifier_mask & (1 << i)],
