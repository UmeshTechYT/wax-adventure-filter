...
def _store_attribute(attrs, key, value):
...
def _walk_attribute_data(obj, attrs):
    """Read flat and nested AtomicAssets attribute formats."""
    if isinstance(obj, dict):
        key_name = obj.get("key") or obj.get("name") or obj.get("attribute")
        if key_name is not None and "value" in obj:
            _store_attribute(attrs, key_name, obj.get("value"))
        if obj.get("trait_type") is not None and "value" in obj:
            _store_attribute(attrs, obj.get("trait_type"), obj.get("value"))
        for key, value in obj.items():
            if key in ("key", "name", "attribute", "trait_type", "value"):
                continue
            if isinstance(value, (dict, list)):
                _walk_attribute_data(value, attrs)
            else:
                _store_attribute(attrs, key, value)
    elif isinstance(obj, list):
        for item in obj:
            _walk_attribute_data(item, attrs)


def get_asset_attributes(asset):
    """Flatten NFT attributes, including nested template.data formats."""
    attrs = {}
    for source in _all_attribute_sources(asset):
        _walk_attribute_data(source, attrs)
    ...
    aliases = {
        ...
        "movcost": [
            "movcost", "move_cost", "movement_cost", "movementcost", "move cost",
            "movecosticon", "movecosticon-immutable", "movecosticon_immutable",
            "movecosticonimmutable", "movecosticon immutable"
        ],
    }
    for canonical, keys in aliases.items():
        for key in keys:
            if key.lower() in attrs and attrs[key.lower()] not in ("", None):
                attrs[canonical] = attrs[key.lower()]
                break
