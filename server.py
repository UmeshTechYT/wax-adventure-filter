...
def fetch_collection_assets(collection_name, max_assets=2000):
    """Fetch NFTs from the collection so suggestions can include NFTs not owned by the current winner."""
...
def top_users_nfts():
    ...
    try:
        ...
        # Build suggestions from the whole collection, not only NFTs already owned by winners.
        # Existing NFTs are used to determine which modifiers are already covered.
        current_assets = []
        current_asset_ids = set()
        for row in winners:
            for asset_id in get_asset_ids_from_row(row):
                if str(asset_id) not in current_asset_ids:
                    current_asset_ids.add(str(asset_id))
                    try:
        ...
        for idx, mod in enumerate(parameters):
            if any(score_suggestion_for_asset(asset, [mod])[1] for asset in current_assets):
                covered_modifier_indexes.add(idx)

        # Discover the collection from the winner's NFT metadata, then scan that collection.
        collection_name = ""
        for asset in current_assets:
            ...
            if not collection_name:
                ...
                if isinstance(collection, dict):
                    collection_name = str(collection.get("collection_name") or collection.get("name") or "")
            if collection_name:
                break

        collection_assets = fetch_collection_assets(collection_name) if collection_name else []
        candidates = []
        seen = set()
        for asset in collection_assets:
            asset_id = str(asset.get("asset_id", ""))
            if not asset_id or asset_id in current_asset_ids or asset_id in seen:
                continue
            seen.add(asset_id)

            new_parameters = [mod for idx, mod in enumerate(parameters) if idx not in covered_modifier_indexes]
            bonus, matches = score_suggestion_for_asset(asset, new_parameters)
            if bonus > 0:
                candidates.append({
                    "asset_id": asset_id,
